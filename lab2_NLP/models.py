...
class BaselineDNN(nn.Module):
    ...
    def __init__(self, output_size, embeddings, trainable_emb=False):
        ...
        # EX5
        self.linear = nn.Linear(2*dim, 1000)  #changes 2* for mean and max pooling
        self.relu = nn.ReLU()
    ...
    def forward(self, x, lengths):
        ...
        # Mean pooling (handle zero lengths)
        sum_embeddings = torch.sum(embeddings, dim=1)
        safe_lengths = lengths.clamp(min=1)
        mean_embeddings = sum_embeddings / safe_lengths.unsqueeze(1)

        # Max pooling (handle padding)
        mask = torch.arange(x.size(1)).unsqueeze(0).to(x.device) < lengths.unsqueeze(1)
        mask = mask.unsqueeze(2).expand_as(embeddings)
        embeddings_masked = embeddings.masked_fill(~mask, float('-inf'))
        max_embeddings = torch.max(embeddings_masked, dim=1).values
        max_embeddings[max_embeddings == float('-inf')] = 0  # Handle all-padding cases
