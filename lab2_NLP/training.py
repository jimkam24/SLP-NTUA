...
def train_dataset(_epoch, dataloader, model, loss_function, optimizer):
    ...
    for index, batch in enumerate(dataloader, 1):
        ...
        if model.__class__.__name__ in ['BaselineDNN', 'LSTM', 'SimpleSelfAttentionModel', 'MultiHeadAttentionModel', 'TransformerEncoderModel']:
...
def eval_dataset(dataloader, model, loss_function):
    ...
    with torch.no_grad():
        for index, batch in enumerate(dataloader, 1):
            ...
            # Step 3 - compute loss (same logic as in training)
            try:
                loss = loss_function(outputs, labels)
            except ValueError:
                bin_labels = torch.nn.functional.one_hot(labels.long(), num_classes=2)
            ...
            # Step 4 - make predictions (class = argmax of posteriors or threshold)
            if outputs.shape[1] == 1:  # binary case with single output neuron
